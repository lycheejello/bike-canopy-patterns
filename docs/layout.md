...
| At 150 px | 15 | 35 | 100 |
| At 300 px | 30 | 70 | 200 |
| At 294 px (dense seat) | **144** | 49 | 101 |
...
The spine on that build also carries a **`DENSE_SPINE_BONUS` of 10 px**, measured
...
fraction change would also move the 300 px bike's spine, and a sparse pixel is
not the same length of bike as a dense one. The two bikes no longer share one
geometry, and this is where that divergence lives.
